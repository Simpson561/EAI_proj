...
class Agent(nn.Module):
    ...
    def forward_gru(self, x, hx):
        x = x.unsqueeze(1)  # [batch_size, 1, obs_dim]
    ...
    def get_value(self, x, hx=None):
        if hx is None:
            hx = torch.zeros(1, x.size(0), 256, device=x.device)
        x, hx = self.forward_gru(x, hx)
        return self.critic(x), hx

    def get_action(self, x, hx=None, deterministic=False):
        if hx is None:
            hx = torch.zeros(1, x.size(0), 256, device=x.device)
        x, hx = self.forward_gru(x, hx)
        action_mean = self.actor_mean(x)
        if deterministic:
            return action_mean, hx
        action_logstd = self.actor_logstd.expand_as(action_mean)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)
        return probs.sample(), hx

    def get_action_and_value(self, x, hx=None, action=None):
        if hx is None:
            hx = torch.zeros(1, x.size(0), 256, device=x.device)
        x, hx = self.forward_gru(x, hx)
        action_mean = self.actor_mean(x)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    action, _ = agent.get_action(eval_obs, deterministic=True)
        ...
        for step in range(0, args.num_steps):
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    final_values[step, torch.arange(args.num_envs, device=device)[done_mask]] = agent.get_value(infos["final_observation"][done_mask])[0].view(-1)
