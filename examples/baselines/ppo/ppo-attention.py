...
class ActorAttentionCritic(nn.Module):
    def __init__(self, envs):
        ...
        self.input_embed = nn.Sequential(
            layer_init(nn.Linear(input_dim, self.embed_dim)),
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(self.embed_dim, 256)),
            nn.Tanh(),
    ...
    def forward(self, x):
        ...
        # 特征聚合
        x = self.feature_pool(x).squeeze(1)
...
if __name__ == "__main__":
    ...
    agent = ActorAttentionCritic(envs).to(device)
