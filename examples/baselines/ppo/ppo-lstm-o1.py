...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = layer_init(nn.Linear(128, self.action_size), std=0.01 * np.sqrt(2))
        self.actor_logstd = nn.Parameter(torch.ones(1, self.action_size) * -0.5)
...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if iteration % args.eval_freq == 1:
            ...
            eval_done = torch.zeros(args.num_eval_envs, device=device)
            eval_metrics = defaultdict(list)
            num_episodes = 0
            for _ in range(args.num_eval_steps):
                with torch.no_grad():
                    action, eval_lstm_state = agent.get_action(eval_obs, eval_lstm_state, eval_done, deterministic=True)
                    eval_obs, eval_rew, eval_terminations, eval_truncations, eval_infos = eval_envs.step(action)
                    eval_done = torch.logical_or(eval_terminations, eval_truncations).to(torch.float32)
        ...
        for step in range(0, args.num_steps):
            ...
            if "final_info" in infos:
                ...
                with torch.no_grad():
                    final_values[step, torch.arange(args.num_envs, device=device)[done_mask]] = agent.get_value(infos["final_observation"][done_mask], next_lstm_state, next_done).view(-1)
